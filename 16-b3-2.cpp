...
{
	num = 2150000 + teacher.num % 1000;  // 工号转学号规则
	strcpy(name, "学生");
	strncat(name, teacher.name + 4, 10);  // 姓名转换规则
	sex = teacher.sex;  // 性别转换规则
	strcpy(addr, teacher.addr);  // 地址转换规则
	strcat(addr, "101室");
...
{
	Teacher teacher;
	teacher.num = 21000 + num % 1000;  // 学号转工号规则
	strcpy(teacher.name, "教师");
	strncat(teacher.name, name + 4, 10);  // 姓名转换规则
	teacher.sex = sex;  // 性别转换规则
	strcpy(teacher.addr, addr);  // 地址转换规则
	strcat(teacher.addr, "电信学院");
