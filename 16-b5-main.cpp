...
int memory_speed_test(int op)
{
	...
	if ((temp = new(nothrow) char[MAX_TEMP]) == NULL) {
...
#if !(defined(__linux) || defined(__linux__))
			cout << "    老师的机器（CPU：Intel i5-9400F , 内存：DDR4 2666MHz）运行VS2022-Debug-x86编译的程序，大约耗时190-210秒" << endl;
#else
			cout << "    Linux服务器下运行，大约耗时140-160秒（如果多人同时测试，偏差可能较大）" << endl;
#endif
		}
		else {
			cout << "内存性能测试" << ((op==1) ? "(s1 += str方式)":"(s1.append()方式)") << endl;
#if !(defined(__linux) || defined(__linux__))
			cout << "    老师的机器（CPU：Intel i5-9400F , 内存：DDR4 2666MHz）运行VS2022-Debug-x86编译的程序，大约耗时90-110秒" << endl;
#else
...
//		const char* str3 = "tong", str4[] = "ji";
//
//		cout << "append()简单测试(TString类和TString类、字符指针、字符数组、字符)" << endl;
//
//		s1.append(s2);
//		cout << "s1应为tongji，实际输出：" << s1 << endl;
//
